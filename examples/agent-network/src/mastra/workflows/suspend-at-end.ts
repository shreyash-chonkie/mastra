...
const researchStep = new Step({
  ...
  execute: async ({ context, mastra }) => {
    ...
    const result = await researchAgent.stream([
      ...
      {
        role: 'assistant',
        content: `Research: ${researchAgentOutput?.research}`,
      },
      {
        role: 'assistant',
        content: `Fact check: ${factCheckAgentOutput?.factCheck}`,
      },
...
const factCheckStep = new Step({
  ...
  execute: async ({ context, mastra }) => {
    ...
    const researchResults = context?.getStepResult(researchStep);
    const editorResults = context?.getStepResult(editorStep);
    const result = await factCheckAgent.stream([
      ...
      { role: 'assistant', content: `Research: ${researchResults?.research}` },
      { role: 'assistant', content: `Editor: ${editorResults?.editor}` },
...
const editorStep = new Step({
  ...
  execute: async ({ context, mastra }) => {
    ...
    const factCheckResults = context?.getStepResult(factCheckStep);
    const researchResults = context?.getStepResult(researchStep);
    const editorResults = context?.getStepResult(editorStep);
    const result = await editorAgent.stream([
      {
        role: 'assistant',
        content: `Fact check: ${factCheckResults?.factCheck}`,
      },
      { role: 'assistant', content: `Research: ${researchResults?.research}` },
      { role: 'assistant', content: `Editor: ${editorResults?.editor}` },
      {
...
const startStep = new Step({
  ...
  execute: async ({ context, suspend }) => {
    ...
    const { inputData } = context;
    console.log('human input step', inputData);
    const prevOutput = context?.getStepResult(startStep);
    if (!prevOutput?.feedback) {
      return { feedback: 'research' };
    }
...
export const agentNetworkWorkflowSuspendAtEnd = new Workflow({
  name: 'agent-network-workflow-suspend-at-end',
  triggerSchema: z.object({
    prompt: z.string().describe('The prompt for the agent network'),
  }),
});

const startStepOg = new Step({
  id: 'end',
  execute: async ({ context, suspend }) => {
    console.log('yo');
    return {};
...
agentNetworkWorkflowSuspendAtEnd
  .step(startStep)
  .after(startStep)
  .step(researchStep, {
    when: async ({ context }) => {
