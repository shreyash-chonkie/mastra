...
const postPlanActivites = createStep({
  id: 'post-plan-activites',
  description: 'Post plan activities',
  inputSchema: z.object({ activties: z.string() }),
  outputSchema: z.object({
    activities: z.string(),
  }),
  execute: async ({ inputData }) => {
    return {
      activities: inputData?.activties,
    };
  },
});
...
const getForecaseSchema = createStep({
  id: 'get-forecast-schema',
  description: 'Gets the forecast schema',
  inputSchema: forecastSchema,
  outputSchema: forecastSchema,
  execute: async ({ inputData }) => {
    return inputData;
...
const planIndoorActivitiesWorkflow = createWorkflow({
...
const planBothWorkflow = createWorkflow({
  ...
  steps: [planActivities, planIndoorActivitiesWorkflow, sythesizeStep],
})
  .parallel([planActivities, planIndoorActivitiesWorkflow])
  .then(sythesizeStep)
  .commit();
...
export { weatherWorkflow as branchingWorkflow };
