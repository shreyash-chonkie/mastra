...
import { Plus, Network, Users, Brain, Info } from 'lucide-react';
...
function Networks() {
  ...
  const networkListData = Object.entries(networks).map(([key, network]) => ({
    id: key,
    name: network.name,
    agents: network.agents?.length || 0,
    provider: network.routingModel?.provider,
    modelId: network.routingModel?.modelId,
  }));
                  ...
                  cell: ({ row }) => (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <Badge className="border-none text-xs">{row.original.modelId}</Badge>
                      </div>
                    </button>
                  ),
                },
