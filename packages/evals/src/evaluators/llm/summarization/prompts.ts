...
export const REASON_TEMPLATE = `
    ...
    Final Score: {score}
    Alignment Outcomes: {alignmentOutcomes}
    Coverage Outcomes: {coverageOutcomes}
...
export function generateQuestionsPrompt({ input }: { input: string }) {
  return QUESTIONS_TEMPLATE.replace('{input}', input);
}
...
export function generateAnswersPrompt({
  ...
  return ANSWERS_TEMPLATE.replace('{input}', input)
    .replace('{output}', output)
    .replace('{questions}', JSON.stringify(questions));
}

export function generateAlignmentPrompt({ input, context }: { input: string; context: string[] }) {
  return ALIGNMENT_TEMPLATE.replace('{input}', input).replace('{context}', JSON.stringify(context));
}
...
export function generateReasonPrompt({
  ...
  return formatter(template, {
    ...
    alignmentScore: String(eval_result?.details?.alignmentScore),
    coverageScore: String(eval_result?.details?.coverageScore),
    alignmentOutcomes: JSON.stringify(eval_result?.details?.alignmentOutcomes),
    coverageOutcomes: JSON.stringify(eval_result?.details?.coverageOutcomes),
  });
}
...
export async function generateEvaluationPrompt({
  ...
  const questions = questionsResult.object.questions;

  // Generate answers based on the output
  const answersPrompt = generateAnswersPrompt({ input, output, questions });
  ...
  const answers = answersResult.object.answers;

  // Create coverage outcomes
  const coverageOutcomes = questions.map((question, i) => ({
    question,
    outcome: answers[i] === 'yes' ? 'yes' : 'no',
  }));
  ...
  return formatter(template, {
    ...
    output,
    ...settings,
    context: JSON.stringify(coverageOutcomes),
  });
}
