...
import { Completeness } from './index';
...
describe('CompletenessMetric', () => {
  ...
  describe('edge cases', () => {
    ...
    it('should handle empty original string', async () => {
      const result = await evaluator.score({ input: '', output: 'some text' });
    ...
    it('should handle null and undefined inputs', async () => {
      ...
      await expect(evaluator.score({ input: null, output: '' })).rejects.toThrow();
      // @ts-expect-error Testing invalid input
      await expect(evaluator.score({ input: '', output: undefined })).rejects.toThrow();
    });
  });
  ...
  describe('special cases', () => {
    ...
    it('should handle repeated elements', async () => {
      const result = await evaluator.score({ input: 'cat cat cat cats', output: 'cat cats' });
