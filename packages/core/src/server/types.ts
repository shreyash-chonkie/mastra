...
export type MastraAuthConfig = {
   ...
   * Public paths for the server
   */
  authenticateToken?: (token: string, request: HonoRequest) => Promise<any>;

  /**
   * Authorization function for the server
   */
  authorize?: (path: string, method: string, user: any, context: ContextWithMastra) => Promise<boolean>;
  ...
  rules?: {
     ...
     */
    condition?: (user: any) => Promise<boolean> | boolean;
    /**
     * Allow the rule
     */
    allow?: boolean;
