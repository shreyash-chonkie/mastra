...
describe('AgentNetwork', () => {
  ...
  it('should initialize with the correct properties', () => {
    ...
    expect(network.model).toEqual(routingModel); // model is set from routingModel
  ...
  it('should generate responses through multiple agents', async () => {
  ...
  it.only('should stream responses through multiple agents', async () => {
    // Set up network state to track agent calls
    network.setCurrentState({
      state: new NetworkState({}),
      lastResult: '',
    ...
    const streamResult = await network.stream(
      'I need a comprehensive analysis of different soda brands, their ingredients, market trends, and a summary of which ones might be healthiest.',
    );
