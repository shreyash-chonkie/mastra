...
> {
  ...
  constructor(config: Config<TAgents, TWorkflows, TVectors, TTTS>) {
    ...
    let logger: Logger = config.logger ? config.logger : (noopLogger as unknown as Logger);
    this.#logger = logger;
...
`);
    ...
    if (config?.tts) {
      let tts: Record<string, MastraTTS> = {};
  ...
  public setLogger({ logger }: { logger: Logger }) {
    this.#logger = logger;
