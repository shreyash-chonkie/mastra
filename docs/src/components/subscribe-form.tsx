...
const buttonCopy = ({
  ...
  loading: <Spinner className="w-4 h-4 !duration-300" color={isDark ? '#000' : '#fff'} />,
...
export const SubscribeForm = ({
  ...
  return (
    <Form {...form}>
      <form
        className={cn('mt-8 items-end flex flex-col md:flex-row w-full gap-2 ', className)}
        onKeyDown={e => {
          if (e.key === 'Enter') {
            e.preventDefault();
            ...
            <FormItem className="flex-1 w-full">
              {showLabel ? (
                <T id="components.subscribe_form.0">
                  <FormLabel>Mastra Newsletter</FormLabel>
                </T>
              ) : null}
              ...
              <span className="md:absolute flex gap-2 items-center">
                {form.formState.errors.email && <AlertCircle size={12} className="text-red-500" />}
