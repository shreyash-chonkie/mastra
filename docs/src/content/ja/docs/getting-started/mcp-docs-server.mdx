...
**新規**プロジェクトの場合、MCP Docsサーバーはインストール中に[インタラクティブ](/docs/getting-started/installation#interactive)セットアッププロンプトを通じて、または[非インタラクティブ](/docs/getting-started/installation#non-interactive)コマンドで`-m`フラグを指定することで追加できます。


## 手動インストール

既存のプロジェクトにMCP Docsサーバーを追加するには、手動でインストールします。

- **Cursor**: プロジェクトルートの`.cursor/mcp.json`、またはグローバル設定の場合は`~/.cursor/mcp.json`を編集します
