...
現在のバージョンの統合では、`MastraClient`を使用してMastraエージェントをCopilotKitのAGUI形式にフォーマットします。
...
このセットアップにより、MastraサーバーでCopilotKitが実行されるようになりました。`mastra dev`でMastraサーバーを起動できます。
...
Mastra Serverを経由せずにAGUIを活用することもできます。
