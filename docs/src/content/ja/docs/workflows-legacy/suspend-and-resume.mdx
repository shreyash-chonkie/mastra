...
サスペンドとレジュームの仕組みをより深く理解するために：

- [Mastra Workflows におけるスナップショットの理解](../../reference/legacyWorkflows/snapshots.mdx) - サスペンドとレジューム機能を支えるスナップショットの仕組みについて学びます
- [ステップ設定ガイド](./steps.mdx) - ワークフロー内のステップ設定について詳しく学びます
- [制御フローガイド](./control-flow.mdx) - 高度なワークフロー制御パターン
- [イベント駆動型ワークフロー](../../reference/legacyWorkflows/events.mdx) - イベントベースのワークフローに関する詳細なリファレンス

## 関連リソース

- 完全な動作例については、[サスペンドと再開の例](../../examples/workflows_legacy/suspend-and-resume.mdx)をご覧ください
- サスペンド／再開APIの詳細については、[Stepクラスリファレンス](../../reference/legacyWorkflows/step-class.mdx)を確認してください
- サスペンドされたワークフローの監視については、[ワークフローの可観測性](../../reference/observability/otel-config.mdx)を参照してください
