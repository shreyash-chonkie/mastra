...
`while`メソッドは、指定された条件が真である限り、ステップを繰り返します。`until`と同じ引数を取ります：
...
ワークフロー全体で包括的な型安全性を確保するには、すべてのステップの型を定義し、それをワークフローに渡すことができます。
これにより、条件付きのコンテキストオブジェクト、および最終的なワークフロー出力におけるステップ結果の型安全性が確保されます。
...
ステップ結果に加えて、ワークフローを開始した元のトリガーデータにアクセスすることもできます：
...
ステップに注入されたデータは`context.inputData`オブジェクトで利用可能で、ステップの`inputSchema`に基づいて型付けされています。
