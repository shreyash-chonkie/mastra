...
異なる音声プロバイダーは、その実装に特有の追加イベントを発行する場合があります：
...
- イベントをサポートしない音声プロバイダーで呼び出された場合、警告を記録し、何もしません
...
- 最良のパフォーマンスを得るために、不要になったイベントリスナーは削除することを検討してください
