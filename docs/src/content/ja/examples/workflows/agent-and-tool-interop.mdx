...
## 天気レポーターエージェントの定義
LLM を活用して天気レポートを天気レポーターのように説明する天気レポーターエージェントを定義します。
...
エージェントとツールをステップとして組み込むワークフローを定義します。
...
ここでは、mastra インスタンスから weather ワークフローを取得し、実行を作成して、必要な inputData を使ってその実行を開始します。
