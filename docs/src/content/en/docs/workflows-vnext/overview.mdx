...
const myStep = createStep({
  ...
  // The resume schema is used to validate the data that can be passed to the step when it is resumed from a suspended state.
  resumeSchema: z.object({
    resumeValue: z.string(),
  }),
  // Optional: Define the suspend schema for step suspension.
  // The suspend schema is used to validate the data that can be passed to the step when it is suspended.
