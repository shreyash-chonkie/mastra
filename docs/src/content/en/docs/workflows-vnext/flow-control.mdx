...
```typescript
// Chain steps to execute in sequence. The output from each step is automatically passed to the next step.
myWorkflow
  .then(step1)
  .then(step2)
  .then(step3)
...
```typescript
// Execute step1 and step2 in parallel. The output from step1 and step2 is passed to step3 as an object.
myWorkflow
