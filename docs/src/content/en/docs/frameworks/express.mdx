...
### Option 1: Use the One-Liner

Run the following command to quickly scaffold the default Weather agent with sensible defaults:

```bash
npx mastra@latest init --dir src --components agents,tools --llm openai --example
```
This sets up:

- A default agent (`weatherAgent`)
- Tooling support
- OpenAI as the provider
- An example prompt
- All files generated under the `src/` directory

### Option 2: Use the Interactive CLI

If you prefer to customize the setup, run the `init` command without flags:

```bash copy
npx mastra@latest init
```

Follow the interactive prompts to configure your Mastra project:
