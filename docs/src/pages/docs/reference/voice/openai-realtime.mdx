...
The OpenAIRealtimeVoice class in Mastra provides real-time voice interaction capabilities using OpenAI's WebSocket-based API. It supports real-time text-to-speech, speech-to-text transcription, voice activity detection, and event-based audio streaming.

## Current Limitations

- **WebSockets Only**: Currently only supports WebSocket connections
- **No Speech Interruption**: Does not support interrupting speech in progress
...
<PropertiesTable
  content={[
    ...
    },
    {
      name: "tools",
      type: "ToolsInput",
      description: "Tools configuration for extending model capabilities.",
      isOptional: true,
    },
    {
      name: "options",
...
Establishes a connection to the OpenAI realtime service. Must be called before using speak, listen, or relay functions.
...
Streams audio data in real-time to the OpenAI service. Useful for continuous audio streaming scenarios like live microphone input.
  ...
  content={[
    {
      name: "audioData",
      type: "NodeJS.ReadableStream",
      description: "Audio stream to relay to the service.",
      isOptional: false,
    },
  ]}
/>
...
Equips the voice instance with a set of tools. Tools allow the model to perform additional actions during conversations.
...
You can also listen to raw OpenAI Realtime events by prefixing with 'openAIRealtime:':
...
- The voice instance must be connected with `huddle()` before using other methods
- Always call `leave()` when done to properly clean up resources