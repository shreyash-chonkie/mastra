...
Condition is used to determine whether a step should transition to the next step.

Mastra workflow is built on top of XState, which allows for complex state transitions in workflows.

A step can have multiple conditions, which are a key/value pair of the next step ID and a condition.
They can be structured in three ways:
